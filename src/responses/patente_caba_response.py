...
class PatenteCabaResponse:
    ...
    def calcular_deuda_acumulada(self, response_data: Dict[str, Any]) -> float:
        deudas = response_data.get("PatenteCaba", {}).get("https://lb.agip.gob.ar/Empadronados/json/GetPosicionesImpagas", {}).get("result", {}).get("deudas", [])
        return sum(deuda.get('importeActualizado', 0) for deuda in deudas)

    def extraer_marca(self, response_data: Dict[str, Any]) -> str:
        cabecera = response_data.get("PatenteCaba", {}).get("https://lb.agip.gob.ar/Empadronados/json/captcha/GetDatos", {}).get("result", {}).get("cabecera", {})
